...
public class FinalFieldTest {
    ...
    * */
    ...
    // private final int NOM_STATIC_NUM; // 선언만 하면 에러남
    private final int NON_STATIC_NUM = 1;

    /*생성자를 이용해서 초기화 함*/
    private final String NON_STATIC_NAME;
    public FinalFieldTest(String NOM_STATIC_NAME) {
        this.NON_STATIC_NAME = NOM_STATIC_NAME;            //알트+인서트->스트럭처
    }

    /*2. static field에 final 사용*/
    /*
    * static 에서도 자바에서 지정한 기본값이 대입되기 때문에 final 키워드 사용시 초기화를 하지 않으면 에러가 발생한다.
    * */
    //private static final int STATIC_NUM; //에러남 //0으로 초기화되기 때문에 이후 값 변경 불가
    private static final int STATIC_NUM = 1;
   /* public FinalFieldTest(int STATIC_NUM) {
        this.STATIC_NUM = STATIC_NUM;*/   //에러발생
    private static final double STATIC_DOUBLE;
    /*
    * 생성자를 이용한 초기화 불가능
    * 생성자는 인스턴스가 생성되는 시점에서 호출이 되기 때문에 그 전에는 초기화가 이루어지지 못한다.
    * 하지만 static 은 프로그램이 start될 때 할당되기 때문에 초기화가 되지 않은 상태로 선언된 것과 동일하여
